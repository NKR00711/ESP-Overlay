...
#define SOCKET_NAME "1311891x141118"
#define BACKLOG 8
int sock,clientD;
...
void Close() {
   // char osVersion[PROP_VALUE_MAX];
    int osVersionLength = android_get_device_api_level();//__system_property_get(Nyanpasu("ro.build.version.sdk"), osVersion);
    //printf("%s",osVersionLength);
    if (clientD > 0) {
        //clientD=2;
        if(osVersionLength>29)
        dup(clientD);
        else
        //dup(clientD);
        close(clientD);
